...
class BehaviorEngine:
    def __init__(self, config: dict) -> None:
        ...
        self.phone = PhoneBehaviorStateMachine(self.config.get("phone", {}), groups, association)
        self.smoking = SmokingBehaviorStateMachine(self.config.get("smoking", {}), groups, association)
    ...
    def update(
        ...
        timestamp_ms = float(now_ms if now_ms is not None else time.monotonic() * 1000.0)
        people = list(tracks)
        objects = list(detections)
        faces = list(face_boxes)
        triggers = self.phone.update(people, objects, faces, frame_shape, timestamp_ms)
        triggers.extend(self.smoking.update(people, objects, faces, frame_shape, timestamp_ms))
