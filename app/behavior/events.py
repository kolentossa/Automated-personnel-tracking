"""Asynchronous unredacted evidence persistence and replaceable callbacks."""
...
class BehaviorEventManager:
    ...
    Input frames are raw and unredacted. Keep the service on a controlled LAN
    and never publish evidence directories without an explicit access policy.
    """

    def __init__(
        self, config: dict, camera_id: str, project_root: Path = PROJECT_ROOT
    ) -> None:
        ...
        self.jpeg_quality = max(
            40, min(100, int(self.config.get("jpeg_quality") or 90))
        )
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._worker, name="behavior-event-writer", daemon=True
        )
        self._thread.start()
    ...
    def emit(self, trigger: BehaviorTrigger, raw_frame: np.ndarray) -> dict:
        event = BehaviorEvent.from_trigger(self.camera_id, trigger)
        event, path_error = self._assign_paths(event)
        payload = event.as_dict()
        payload.update(
            {"persistence_status": "queued", "persistence_error": path_error}
        )
        with self._lock:
            self._recent.appendleft(payload)
            self._counts[event.event_type] += 1
        try:
            self._queue.put_nowait((payload, raw_frame.copy()))
        except queue.Full:
            self._dropped += 1
    ...
    def recent(self, limit: int = 50) -> List[dict]:
        with self._lock:
            return [
                dict(item)
                for item in list(self._recent)[: max(1, min(200, int(limit)))]
            ]
    ...
    def _worker(self) -> None:
        while not self._stop_event.is_set() or not self._queue.empty():
            ...
            payload, frame = item
            errors: List[str] = (
                [str(payload.get("persistence_error"))]
    ...
    def _persist_images(
        self, payload: dict, frame: np.ndarray, errors: List[str]
    ) -> None:
        if payload.get("snapshot_path"):
            self._write_image(
                payload["snapshot_path"], frame, "unannotated snapshot", errors
            )
    ...
    def _write_image(
        self, relative_path: str, frame: np.ndarray, label: str, errors: List[str]
    ) -> None:
        try:
    ...
    def _assign_paths(self, event: BehaviorEvent) -> Tuple[BehaviorEvent, str]:
        ...
        try:
            snapshot_dir = self._safe_path(
                str(self.config.get("snapshot_dir") or "data/behavior_events/snapshots")
            )
            raw = (
                self._relative(snapshot_dir / f"{stem}_evidence.jpg")
                if self.save_unannotated
                else None
            )
            annotated = (
                self._relative(snapshot_dir / f"{stem}_annotated.jpg")
    ...
    def _safe_path(self, value: str) -> Path:
        ...
        if resolved != self.project_root and self.project_root not in resolved.parents:
            raise ValueError(
                f"Evidence path must stay inside project directory: {value}"
            )
        return resolved
...
def _draw_event_annotation(frame: np.ndarray, payload: dict) -> np.ndarray:
    colors: Dict[str, Tuple[int, int, int]] = {
        ...
        "head_region": (190, 150, 30),
        "mouth_region": (210, 120, 30),
    }
    ...
    cv2.putText(
        frame, title, (12, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (20, 20, 230), 2
    )
    return frame
