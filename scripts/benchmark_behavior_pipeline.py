...
def main() -> int:
    ...
    parser.add_argument(
        "--video", help="Use a local video instead of the configured camera"
    )
    parser.add_argument(
        "--json-output", help="Optional path for the machine-readable result"
    )
    ...
    if args.video:
        camera_config.update(
            {"source_type": "video", "video_file": args.video, "auto_detect": False}
        )

    camera = CameraSource(camera_config)
    detector = PersonDetector(
        config.get("detector", {}), fallback_config=config.get("detection", {})
    )
    behavior_detector = BehaviorObjectDetector(
        behavior_config.get("models", {}).get("behavior", {})
    )
    ...
    try:
        for frame_index in range(args.frames):
            ...
            if frame_index % detect_interval == 0 or not detector_has_run:
                ...
                last_people = [
                    item
                    for item in last_scene
                    if item.label == "person" or item.class_id == 0
                ]
            ...
            behavior_inference_ms = float(
                behavior_detector.last_profile.get("inference_ms", 0.0) or 0.0
            )
            ...
            display = frame.copy()
            samples["privacy_ms"].append(0.0)
            result = behavior.update(
                tracks,
                list(last_scene) + auxiliary,
                frame.shape[:2],
            ...
            if (
                stream_width > 0
                and stream_height > 0
                and display.shape[1::-1] != (stream_width, stream_height)
            ):
    ...
    result = {
        ...
        "input_resolution": {"width": frame_size[0], "height": frame_size[1]}
        if frame_size
        else None,
        ...
        "npu_load_percent_average": round(statistics.fmean(npu_samples), 2)
        if npu_samples
        else None,
        ...
        "behavior_detect_every_n_frames": int(
            behavior_detector.snapshot().get("behavior_model_detect_every_n_frames")
            or 1
        ),
...
def _read_npu_load() -> List[float]:
    ...
    for path in candidates:
        ...
        if not values:
            values = [
                float(value)
                for value in re.findall(r"(?:^|\s)(\d+(?:\.\d+)?)@\d+Hz", text)
            ]
        if values:
            return values
    return []
...
def _percentile(values: List[float], percentile: int) -> float:
    ...
    index = max(
        0, min(len(ordered) - 1, int(round((percentile / 100.0) * (len(ordered) - 1))))
    )
